# Drift Radar – »Zeig mir Magenpellets für Pferde ohne künstliche Zusätze.«

**Prompt ID:** `pr_892d418a-dd4f-4098-9dc5-c2d0c8be6d83`  ·  **Topic:** Magen-Darm-Gesundheit Pferd  ·  **Volume:** – (Peec bucket)  ·  **Date range:** 2026-04-27 → 2026-04-29

## Engine visibility

- **ChatGPT:** 0 %
- **Gemini:** 0 %
- **AI Overview:** 0 %

**Divergence score:** 0.00  (range 0 – 1; 0 = engines agree, 1 = total divergence)

**Silence type:** `own_only`

## Who is cited instead

- **St. Hippolyt** – top visibility 67 %  (Gemini 67 %)
- **Havens** – top visibility 33 %  (Gemini 33 %)
- **Höveler** – top visibility 33 %  (ChatGPT 33 %)

## Citation source mix

- **Reference:** 4.8 %
- **Institutional:** 2.5 %
- **Corporate:** 90.2 %
- **Other:** 2.5 %

**Top cited domains**

- `sapodoris.de` – CORPORATE, retrieved 50 % of chats
- `pferdeliebehealthy.de` – CORPORATE, retrieved 50 % of chats
- `discountfutter.shop` – CORPORATE, retrieved 33 % of chats
- `emma-care.de` – CORPORATE, retrieved 33 % of chats
- `heimtierwelt-shop.de` – CORPORATE, retrieved 33 % of chats
- `hunde-kausnacks.de` – CORPORATE, retrieved 33 % of chats

## Suggested article

- **Format:** Head-to-head comparison page
- **Length:** 1 200 – 1 800 words
- **Headline starting point:** »Pferdegold vs St. Hippolyt: Zeig mir Magenpellets für Pferde ohne künstliche Zusätze.«

_The deterministic suggestion above is generated from silence type and search-volume bucket. Treat it as a starting point, not a brief lock-in._

## Concrete moves

- **Head-to-head page.** Position Pferdegold directly against St. Hippolyt, Havens, Höveler for this prompt.
- **Retailer placement.** Inspect top-cited domains; ensure product assets and schema tags are present.
- **Schema.org/Product markup** matching competitor-equivalent spec fields.
- **Editorial earned mentions.** Pitch study-backed angles to domains in the Editorial source class.
- **Re-measure in 14 days** to confirm visibility lift.

---
Source: Peec AI MCP · Drift Radar · Built for the Peec AI MCP Challenge 2026 · #builtWithPeec