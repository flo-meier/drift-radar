# Drift Radar – »Pferdefutter zur Gelenkunterstützung ohne künstliche Aromen kaufen.«

**Prompt ID:** `pr_ce22dac0-782c-42ea-a6e4-2c595108b4d6`  ·  **Topic:** Gelenkunterstützung für Pferde  ·  **Volume:** – (Peec bucket)  ·  **Date range:** 2026-06-26 → 2026-06-28

## Engine visibility

- **ChatGPT:** 0 %
- **Gemini:** 0 %
- **AI Overview:** 0 %

**Divergence score:** 0.00  (range 0 – 1; 0 = engines agree, 1 = total divergence)

**Silence type:** `own_only`

## What the engines say

### ChatGPT – visibility 0 %

**Brands cited (tracked):** #1 Pavo

- _brand_ – Fohlenhof Pferdefutter offers joint support
- _brand_ – Equine Nutritional Solutions offers joint support
- _brand_ – Pavo offers joint support
- _criterion_ – These brands offer natural joint support

### Gemini – visibility 0 %

**Brands cited (tracked):** #1 Pferdegold

- _brand_ – Pferdegold Gelenke is a joint support product
- _substance_ – Contains MSM for joint support
- _substance_ – Contains Glucosamin for joint support
- _criterion_ – Made with natural ingredients like flaxseed and carrot

### AI Overview – visibility 0 %

_Prompt did not run on AI Overview in the sampled date range – absence is itself a drift signal._

## Stable vs. divergent concepts

_No claims overlap across two or more engines. Every engine answers with its own concepts – the narrative is fully divergent._

## Citation source mix

- **Corporate:** 100.0 %

**Top cited domains**

- `stroeh.de` – CORPORATE, retrieved 50 % of chats
- `raiffeisenmarkt.de` – CORPORATE, retrieved 50 % of chats
- `pferdefuttershop.de` – CORPORATE, retrieved 50 % of chats
- `stallbedarf24.de` – CORPORATE, retrieved 50 % of chats
- `mambocare.de` – CORPORATE, retrieved 33 % of chats
- `pferdefutter.de` – CORPORATE, retrieved 33 % of chats

## Suggested article

- **Format:** Head-to-head comparison page
- **Length:** 1 200 – 1 800 words
- **Headline starting point:** »Pferdegold vs Eggersmann: Pferdefutter zur Gelenkunterstützung ohne künstliche Aromen kaufen.«

_The deterministic suggestion above is generated from silence type and search-volume bucket. Treat it as a starting point, not a brief lock-in._

## Concrete moves

- **No stable concepts detected.** Decide which narrative to own, seed across owned (product page), editorial (blog) and citeable (study/PDF) assets in parallel.
- **Own-only silence.** Eggersmann, Höveler, St. Hippolyt already own this prompt. Priorities: (1) head-to-head page, (2) retailer placements on cited domains, (3) schema.org/Product markup matching competitor spec fields.

---
Source: Peec AI MCP · Drift Radar · Built for the Peec AI MCP Challenge 2026 · #builtWithPeec